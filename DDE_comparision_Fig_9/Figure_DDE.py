...
################################################################
delays = np.arange(0, 32, 0.5)
with open('./final_dde.pkl', 'rb') as f:  # Use 'wb' for binary mode
    data_dde = pickle.load(f)
peaks_psd_dde = data_dde['peaks_psd']
peaks_psd_dde[np.where(peaks_psd_dde >900)] = np.nan
min_autoc_dde = data_dde['min_autoc']
max_autoc_dde = data_dde['max_autoc']
################################################################
################################################################
plt .close('all')
...
################################################################
################################################################
ax1.plot(delays, 1000./peaks_psd_dde, '.-k', label = 'Delayed DE')
ax2.plot(delays, (max_autoc_dde - min_autoc_dde)/2, '.-k')
ax2.plot(delays, (min_autoc_dde - max_autoc_dde)/2, '.-k')
################################################################
ax1.plot(delays, 1000./peaks_psd_nn, '.-r', label = 'Hubs neural network ')
ax22.plot(delays, (max_autoc_nn - min_autoc_nn)/2, '.-r')
ax22.plot(delays, (min_autoc_nn - max_autoc_nn)/2, '.-r')
################################################################
...
ax22.set_xlim(-0.9, 32.5)
################################################################
ax1.set_ylabel('Period PSD peak (ms)', fontsize = 13)
ax2.set_ylabel('Autoc amplitude DDE\n (Spk/s)', fontsize = 13)
ax22.set_ylabel('Autoc amplitude NN\n (Spk/s)', fontsize = 13, color = 'r', rotation = -90, labelpad = 25)

ax2.set_yticks([-10, -5, 0, 5, 10])
ax2.set_yticklabels(['-10', '-5', '0', '5', '10'])

ax22.set_yticks([-1.5, -0.5, -1, 0, 0.5, 1, 1.5])
ax22.set_yticklabels(['-1.5', '-1', '-0.5', '0', '0.5', '1', '1.5'])
################################################################
fig.subplots_adjust(left = 0.07, bottom = 0.14, right = 0.91, top = 0.96)
plt.savefig('delay_DDE_NN.png', dpi = 400)
plt.savefig('delay_DDE_NN.pdf')
################################################################
