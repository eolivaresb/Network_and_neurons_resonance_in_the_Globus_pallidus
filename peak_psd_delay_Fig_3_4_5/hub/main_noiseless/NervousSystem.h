...
class NervousSystem {
    public:
        ...
        ////////////// Buffer  /////////////////////////
        int BufferSize;
        TMatrix<double> delay;
        void Set_delay(int pre, int post, double value) {delay[pre][post] = value;};
        TMatrix<double> ConductanceBuffer;
        ///////////////////////////////////////////////
        ////////////// PRCs and their  interpolation matrix

        int prcbins;
        TVector<TVector<double>> prc_vectors;
        double PrcEvalInterp(int neuron, double x);
        void LoadPrcVectors();
        ...
        //// Poisson process rates for excitatory synapses to compensate inhibitory network activity
        TVector<double> Conductance_Glu;
        TVector<double> Rates_Glu;
        double tauG_Glu, Erev_Glu, EpscRef;

        //// Neurons states and rate adaptation parameterrs
        double afactor, tau_adapt;
        TVector<double> Phases, Phases_pre, W, CV, externalinputs, W_adapt;
